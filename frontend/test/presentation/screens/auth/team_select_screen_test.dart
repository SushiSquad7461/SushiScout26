...
void main() {
  ...
  ProviderContainer container() {
    ...
    c.read(authProvider.notifier).state = c
        .read(authProvider)
        .copyWith(status: AuthStatus.needsTeamSelection, userId: 'alice');
    return c;
  }
  ...
  testWidgets('filters non-digit characters as they are typed', (tester) async {
    final c = container();
  ...
  testWidgets(
    'rejects a leading-zero team number inline; does not call createTeam',
    (tester) async {
      final c = container();
      await tester.pumpWidget(wrap(c));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'truncates a 6-digit team number to 5 digits and still calls createTeam',
    (tester) async {
      when(
        mockTeamRepository.createTeam(
          name: anyNamed('name'),
      ...
      when(mockAuthService.forceRefreshClaims()).thenAnswer(
        (_) async => {
          'teams': {'newTeam': 'admin'},
        },
      );
